  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0A", padding: 24, fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 28 }}>
        {[
          { label: "En attente", value: String(readyOrders.length), sub: totalPending.toFixed(2) + " DA", color: ORANGE },
          { label: "Payees", value: String(paidOrders.length), sub: totalPaid.toFixed(2) + " DA", color: GREEN },
          { label: "Total encaisse", value: totalPaid.toFixed(2) + " DA", sub: paidOrders.length + " commandes", color: "#0096FF" },
        ].map((s) => (
      ...
      <div>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: TEXT, marginBottom: 14 }}>Commandes payees - Total: {totalPaid.toFixed(2)} DA</h2>
        {paidOrders.length === 0 ? (
          ...
          <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: 16, overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 100px", padding: "12px 16px", background: "rgba(0,210,100,0.08)", borderTop: "2px solid rgba(0,210,100,0.3)" }}>
              ...
              <span style={{ textAlign: "right", fontWeight: 800, color: GREEN, fontSize: 16 }}>{totalPaid.toFixed(2)} DA</span>
            </div>
          </div>
        )}
      </div>
